...
namespace
{
	...
	static bool EffectRefHasStep(const FTCGCardEffectRef& EffectRef, ETCGEffectStepType StepType)
	{
		...
		{
			if (Step.StepType == StepType) return true;
		}
		return false;
	}
	...
	static bool DiscardSourceReturnTargetUnitToHandDrawIfTwoMaterials(ATCG_GameState* GameState, const FTCGEffectChainEntry& ChainEntry)
	{
		...
		if (!SourceCard || SourceCard->OwnerPlayerIndex != ChainEntry.ControllerPlayerIndex || SourceCard->Location != ETCGCardLocation::Hand) return false;
		if (!TargetCard || TargetCard->Location != ETCGCardLocation::Board || TargetCard->OwnerPlayerIndex != ChainEntry.ControllerPlayerIndex) return false;

		const FName SourceDefinitionId = SourceCard->CardDefinitionId;
		const FName TargetDefinitionId = TargetCard->CardDefinitionId;
		if (!GameState->MoveCardToLocation(ChainEntry.SourceCardInstanceId, ETCGCardLocation::Graveyard)) return false;
		...
		UE_LOG(LogTemp, Warning, TEXT("TCG Effect: Unit save Source=%s Target=%s Discarded=true Returned=%s Materials=%d Drawn=%d"), *SourceDefinitionId.ToString(), *TargetDefinitionId.ToString(), bReturnedUnit ? TEXT("true") : TEXT("false"), MaterialCount, DrawnCount);
	...
	static bool DiscardSourceDetachUpToTwoMaterialsFromTarget(ATCG_GameState* GameState, const FTCGEffectChainEntry& ChainEntry)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("TCG Effect: Discard source detach Source=%s Target=%s Discarded=true Detached=%d"), *SourceDefinitionId.ToString(), *TargetDefinitionId.ToString(), DetachedCount);
		return true;
	}

	static bool TryHandSaveReplacement(ATCG_GameState* GameState, const FGuid& TargetTopCardInstanceId)
	{
		const FTCGCardInstance* TargetCard = GameState ? GameState->FindCardInstance(TargetTopCardInstanceId) : nullptr;
		if (!GameState || !TargetCard || TargetCard->Location != ETCGCardLocation::Board) return false;
		...
		for (const FTCGCardInstance& HandCard : HandCards)
		{
			...
			{
				if (!GameState->DoesCardEffectMatchTrigger(EffectRef, ETCGEffectTrigger::OnYourUnitWouldBeDestroyedByCardEffect)) continue;
				if (!EffectRefHasStep(EffectRef, ETCGEffectStepType::DiscardSourceReturnTargetUnitToHandDrawIfTwoMaterials)) continue;
	...
	static bool DestroyTargetUnitByCardEffect(ATCG_GameState* GameState, const FTCGEffectChainEntry& ChainEntry, const FTCGEffectStep& Step)
	{
		...
		if (TryHandSaveReplacement(GameState, TargetCardInstanceId)) return true;
...
bool ATCG_GameState::ResolveEffectStep(const FTCGEffectChainEntry& ChainEntry, const FTCGEffectStep& Step, bool bPreviousStepSucceeded)
{
	...
	case ETCGEffectStepType::DestroyTargetUnitByCardEffect:
	{
		...
		if (bLogEffectResolution) UE_LOG(LogTemp, Warning, TEXT("TCG Effect: Step DestroyTargetUnitByCardEffect Player=%d Success=%s"), ChainEntry.ControllerPlayerIndex, bStepSucceeded ? TEXT("true") : TEXT("false"));
		break;
	}
